    ...
    <div class="page-body">
        <div class="container-xl">
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{ route('admin.application-org-types.update', ['locale' => app()->getLocale(), 'application_org_type' => $orgType->id]) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <div class="mb-3">
                            <label class="form-label">Slug</label>
                            <input type="text" class="form-control" name="slug" placeholder="Slug" value="{{ $orgType->slug }}">
                            <x-input-error :messages="$errors->get('slug')" class="mt-2" />
                        </div>
                        @php($trans = $orgType->translations->keyBy('locale'))
