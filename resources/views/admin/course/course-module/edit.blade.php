    ...
    <div class="add_course_basic_info">
        <form action="{{ route('admin.courses.update', ['locale' => app()->getLocale()]) }}" method="post" class="basic_info_update_form course-form" enctype="multipart/form-data">
            ...
            <div class="col-xl-6">
                ...
                @php($trans = $course->translations->keyBy('locale'))
                @foreach(['ru'=>'Русский','kk'=>'Қазақша','en'=>'English'] as $loc => $label)
                <div class="col-xl-12">
                    <div class="add_course_basic_info_imput">
                        <label>Title ({{ $label }})</label>
                        <input type="text" name="translations[{{ $loc }}][title]" placeholder="Title ({{ $label }})" value="{{ $trans[$loc]->title ?? '' }}">
                    </div>
                </div>
                <div class="col-xl-12">
                    <div class="add_course_basic_info_imput">
                        <label>Slug ({{ $label }})</label>
                        <input type="text" name="translations[{{ $loc }}][slug]" placeholder="Slug ({{ $label }})" value="{{ $trans[$loc]->slug ?? '' }}">
                    </div>
                </div>
                <div class="col-xl-12">
                    <div class="add_course_basic_info_imput">
                        <label>SEO Description ({{ $label }})</label>
                        <input type="text" name="translations[{{ $loc }}][seo_description]" placeholder="SEO Description ({{ $label }})" value="{{ $trans[$loc]->seo_description ?? '' }}">
                    </div>
                </div>
                <div class="col-xl-12">
                    <div class="add_course_basic_info_imput mb-0">
                        <label>Description ({{ $label }})</label>
                        <textarea rows="4" name="translations[{{ $loc }}][description]" placeholder="Description ({{ $label }})" class="editor">{{ $trans[$loc]->description ?? '' }}</textarea>
                    </div>
                </div>
                @endforeach
            </div>
