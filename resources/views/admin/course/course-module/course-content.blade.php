    ...
    <div class="tab-pane fade show active" id="pills-contact" role="tabpanel" aria-labelledby="pills-contact-tab" tabindex="0">
        ...
        <div class="add_course_content">
            <div class="add_course_content_btn_area d-flex flex-wrap justify-content-between mt-3">
                <a class="common_btn chapter-modal-btn btn btn-primary" href="#" data-id="{{ $courseId }}" data-url="{{ route('admin.course-content.create-chapter', ['locale' => app()->getLocale(), 'course' => $courseId]) }}"> Add New Chapter</a>
...
document.addEventListener('DOMContentLoaded', function() {
  document.querySelectorAll('.chapter-modal-btn').forEach(function(btn){
    btn.addEventListener('click', function(e){
      e.preventDefault();
      var url = this.getAttribute('data-url');
      if (!url) return;
      var modal = document.getElementById('dynamic-modal');
      var content = modal.querySelector('.dynamic-modal-content');
      ...
      fetch(url, { headers: { 'X-Requested-With': 'XMLHttpRequest' }})
        ...
        .catch(function(){ content.innerHTML = '<div class="modal-content p-4">Failed to load</div>'; });
    });
  });
});
</script>
